...
class SeederSpider(scrapy.Spider):
    ...
    def filter_link(self,link):
        pattern_list =["instagram","spotify","deezer","shop","configure","github","vimeo","apple","twitter","facebook","google","whatsapp","tiktok","pinterest", "klarna", "jobs","linkedin","xing", "mozilla","youtube", "gebrauchtwagen", "neufahrzeug"]
        flag = False
...
def run_crawler():
    ...
    process2 =  CrawlerProcess({
        'USER_AGENT': 'Mozilla/5.0',
        'FEED_EXPORT_ENCODING': 'utf-8',
        'FEEDS': {'files/raw_topics.json': {'format': 'json','encoding': 'utf8','fields': ['TOPIC','URL', 'URL_TEXT']}}
        })

    process.crawl(SeederSpider, r'files\Seed.feather')
    #process2.crawl(TopicSpider,'DE')
    process.start()
    #process2.start()
